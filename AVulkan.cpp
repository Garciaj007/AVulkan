...
#pragma endregion
	...
	void GetVulkanLayerSupport(std::vector<const char*>& layers)
	{
		layers.clear();
	...
	QueueFamilyIndices GetVulkanQueueFamilies(const VkPhysicalDevice& device, const VkSurfaceKHR& vkSurface)
	{
	...
	bool IsVulkanDeviceCompatible(const VkPhysicalDevice& device, const VkSurfaceKHR& vkSurface)
	{
		QueueFamilyIndices indices = GetVulkanQueueFamilies(device, vkSurface);
		return indices.isComplete();
	}

	void GetVulkanPhysicalDevice(const VkInstance& instance, const VkSurfaceKHR& vkSurface, VkPhysicalDevice& outDevice)
	{
		...
		for (const auto& physicalDevice : physicalDevices)
		{
			if (IsVulkanDeviceCompatible(physicalDevice, vkSurface))
			{
	...
	void CreateVulkanLogicalDevice(VkPhysicalDevice& physicalDevice, const VkSurfaceKHR& vkSurface, const std::vector<const char*>& layers, VkDevice& outLogicalDevice, VkQueue& outGraphicsQueue, VkQueue& outPresentQueue)
	{
		QueueFamilyIndices indices = GetVulkanQueueFamilies(physicalDevice, vkSurface);
	...
	void GetVulkanPresentationMode(const VkSurfaceKHR& vkSurface, const VkPhysicalDevice& physicalDevice, VkPresentModeKHR& outMode)
	{
		uint32_t modeCount;
		if (vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, vkSurface, &modeCount, nullptr) != VK_SUCCESS)
		...
		if (vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, vkSurface, &modeCount, availableModes.data()) != VK_SUCCESS)
	...
	void GetVulkanImageFormat(const VkPhysicalDevice& device, const VkSurfaceKHR& vkSurface, VkSurfaceFormatKHR& outFormat)
	{
		uint32_t count;
		if (vkGetPhysicalDeviceSurfaceFormatsKHR(device, vkSurface, &count, nullptr) != VK_SUCCESS)
		...
		if (vkGetPhysicalDeviceSurfaceFormatsKHR(device, vkSurface, &count, foundFormats.data()) != VK_SUCCESS)
	...
	void CreateVulkanSwapchain(const VkSurfaceKHR& vkSurface, const VkPhysicalDevice& physicalDevice, const VkDevice& device, VkSwapchainKHR& outSwapchain, VkSurfaceFormatKHR& outSurfaceFormat, VkExtent2D& outExtent)
	{
		...
		GetVulkanPresentationMode(vkSurface, physicalDevice, presentMode);
		...
		GetVulkanImageFormat(physicalDevice, vkSurface, imageFormat);
	...
	void CreateVulkanCommandPool(const VkPhysicalDevice& physicalDevice, const VkDevice& device, const VkSurfaceKHR& vkSurface, VkCommandPool& outCmdPool) {
		QueueFamilyIndices queueFamilyIndices = GetVulkanQueueFamilies(physicalDevice, vkSurface);
	...
	struct IRenderer
	{
		enum class API
		{
			None, OpenGL, Vulkan, DX11, DX12
		};

		virtual ~IRenderer() = 0;
		virtual void Draw() = 0;
		virtual void Resize(const SDL_Window* window) = 0;
	};

	struct VulkanRenderer : IRenderer
	{
		VulkanRenderer(const SDL_Window* window)
		{
			const auto _ = const_cast<SDL_Window*>(window);
			GetAndCheckVulkanAPISupport(apiVersion);
			GetVulkanExtensions(_, extensions);
			GetVulkanLayerSupport(layers);
			CreateVulkanInstance(instance, apiVersion, extensions, layers);
			SetupVulkanDebugMessengerCallback(instance, debugMessenger);
			CreateVulkanSurface(_, instance, surface);
			GetVulkanPhysicalDevice(instance, surface, physicalDevice);
			CreateVulkanLogicalDevice(physicalDevice, surface, layers, device, graphicsQueue, presentQueue);
			CreateVulkanSwapchain(surface, physicalDevice, device, swapchain, )
		}
		...
		void Resize(const SDL_Window* window) override
		{
	...
	struct Window
	{
		...
		Window(Props props) : name(std::move(props.name)), size(std::move(props.size)), flags(props.flags), vsync(props.vsync)
		{
			window = SDL_CreateWindow(name.c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, size.w, size.h, flags);
			if (!window) throw std::exception("Could not create SDL Window.", __LINE__);
		}
		...
		virtual SDL_Window* Expose() const { return window; }

		virtual void Resize() { SDL_GetWindowSize(window, &size.w, &size.h); }
	...
	struct Time
	{
		double time, deltaTime, lastFrame;
	};
	...
	struct Application
	{
		virtual void Run()
		{
			...
			while (isRunning)
			{
				...
				time.time = std::chrono::duration<float, std::chrono::seconds::period>(current - start).count();
		...
		static const IRenderer::API RENDER_API = IRenderer::API::Vulkan;
		static Time time;
	...
	protected:
		virtual void Init()
		{
			if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS) < 0)
				throw std::exception("Failed to initialize SDL.", __LINE__);

			try {
				window = CreateWindow(RENDER_API, Window::Props{ "AVulkan", {1200, 800}, SDL_WINDOW_SHOWN, false });
				renderer = CreateRenderer(RENDER_API, window->Expose());
				isRunning = true;
			}
			catch (std::exception e)
			{
				SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "AVulkan Error.", e.what(), nullptr);
				isRunning = false;
			}
		}
			...
			try {
				...
				while (SDL_PollEvent(&event))
				{
					...
					case SDL_WINDOWEVENT:
						...
						{
						case SDL_WINDOWEVENT_CLOSE: isRunning = false;
							return;
						case SDL_WINDOWEVENT_RESIZED: Resize();
							break;
	...
	//! Call this function only once, when creating a renderer...
	std::unique_ptr<IRenderer> CreateRenderer(IRenderer::API api, const SDL_Window* pWindow)
	{
		using _ = IRenderer::API;
		switch (api)
		{
		case _::Vulkan:
		default: return std::unique_ptr<VulkanRenderer>(new VulkanRenderer(pWindow));
		}
	}

	std::unique_ptr<Window> CreateWindow(IRenderer::API api, Window::Props props)
	{
		using _ = IRenderer::API;
		switch (api)
		{
		case _::Vulkan:
		default: props.flags |= SDL_WINDOW_VULKAN;
		}
...
#pragma region raw
	...
	//	CreateVulkanSurface(sdlWindow, vkInstance, vkSurface);

	//	GetVulkanPhysicalDevice(vkInstance, vkSurface, vkPhysicalDevice);

	//	CreateVulkanLogicalDevice(vkPhysicalDevice, vkSurface, layers, vkDevice, vkGraphicsQueue, vkPresentQueue);

	//	CreateVulkanSwapchain(vkSurface, vkPhysicalDevice, vkDevice, vkSwapchain, vkSurfaceFormat, vkExtent);
	...
	//	CreateVulkanCommandPool(vkPhysicalDevice, vkDevice, vkSurface, vkCommandPool);
...
#pragma endregion
	...
	{
		std::cerr << "AVulkan Error: " << e.what() << std::endl;
		return 1;
	}
